...
T<-c("A", "B", "C", "D") #defining opponent's board 
T1<-c("A1", "B1", "C1", "D1") #defining own board
...
for (i in 1:N){
	...
	if (is.element(T[x], c("C","D"))) {
		removex<-T[x]
		T=setdiff(T, removex) 					# remove bomb C or D after it's hit
		DMGy<-damage_selection("bomb")			 #select damage of bomb C or D
		y<- target_selection(T1)			 #select target of bomb C or D
		

		if (is.element(T1[y], c("C1","D1"))) {
			removey<-T1[y]
